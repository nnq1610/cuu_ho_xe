...
const Banner = () => {
  ...
  const prices = ['Xe Máy', 'Ô Tô', 'Xe Đạp'];
  const fixedLocations = ['Hà Nội', 'Hải Phòng', 'Thành phố Hồ Chí Minh'];

  const handleSearch = async (event) => {
    event.preventDefault();

    // Chuyển hướng đến trang /searchService với các tham số
    navigate('/searchService', {
      state: {
        vehicleType: selectedPrice,
  ...
  return (
      <div className="flex flex-col items-center md:mr-10 md:ml-10 bg-bg-image-1 bg-contain md:bg-bg-white pb-6">
        <div className="hidden md:block w-full rounded-[50px] h-[580px] overflow-hidden">
          <Swiper spaceBetween={10} slidesPerView={1} loop={true} autoplay={{ delay: 1000, disableOnInteraction: false }} className="w-full h-full">
            <SwiperSlide>
              <img src="/images/img_1.png" alt="Cứu hộ 1" className="w-full h-full object-cover" />
            </SwiperSlide>
            <SwiperSlide>
              <img src="/images/godenbay_ht.jpg" alt="Cứu hộ 2" className="w-full h-full object-cover" />
            </SwiperSlide>
            <SwiperSlide>
        ...
        <div className="bg-white shadow-md w-[300px] sm0:w-[80%] sm:w-[65%] h-[440px] sm:h-[240px] rounded-[30px] mt-5 relative z-[400] md:top-[-130px]">
          <div className="mx-5 my-10">
            <h2 className="text-[24px] md:text-[30px] text-center font-bold ">Bạn lựa chọn dịch vụ cứu hộ nào?</h2>
            <p className="text-[18px] mt-3 md:text-[20px] font-light text-[#101828] text-center">Hơn 100 dịch vụ cứu hộ đang chờ bạn</p>
